...
class BackprojectDepth(nn.Module):
  ...
  def forward(self, depth, inv_K):
    # project pixels into rays
    cam_points = torch.matmul(inv_K[:, :3, :3], self.pix_coords)
    # extend rays into scene
    cam_points = depth.view(self.batch_size, 1, -1) * cam_points
    # make points into homogeneous coordinates
...
class Project3D(nn.Module):
  ...
  def forward(self, points, K, T):
    ...
    # T @ points : transform points to adjacent frame
    # K : project points onto image plane
    cam_points = torch.matmul(P, points)

    # remove homogeneous coordinates (leaves x,y)
