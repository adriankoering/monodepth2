  ...
  def __getitem__(self, index):
    ...
    for i in self.frame_idxs:
      if i == "s":
        ...
        inputs[("color", i, -1)] = self.get_color(folder, frame_index,
                                                  other_side, do_flip)
      else:
    ...
    if do_color_aug:
      color_aug = transforms.ColorJitter.get_params(self.brightness,
                                                    self.contrast,
                                                    self.saturation, self.hue)
    else:
      color_aug = (lambda x: x)

    self.preprocess(inputs, color_aug)  # downsample and augment images

    # remove native resolution from inputs
    for i in self.frame_idxs:
