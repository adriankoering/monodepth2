...
def evaluate(opt):
    ...
    with torch.no_grad():
      for data in dataloader:
        ...
        if opt.post_process:
          ...
          input_color = torch.cat((input_color, torch.flip(input_color, [3])),
                                  0)
        ...
        if opt.post_process:
          ...
          pred_disp = batch_post_process_disparity(pred_disp[:N],
                                                   pred_disp[N:, :, ::-1])
  ...
  for i in range(pred_disps.shape[0]):
    ...
    if opt.eval_split == "eigen":
      ...
      crop = np.array([
          0.40810811 * gt_height, 0.99189189 * gt_height, 0.03594771 * gt_width,
          0.96405229 * gt_width
      ]).astype(np.int32)
